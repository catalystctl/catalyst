...
# Edit .env — set PUBLIC_URL and verify generated secrets
nano .env
...
**Redis** is optional shared infrastructure for caching, coordination, and realtime fan-out (see `docs/redis.md`). The backend reads `REDIS_URL` and degrades to Postgres + process-local state when Redis is unset or unreachable. The Docker stack still requires `REDIS_PASSWORD` (generated by `install.sh`); Redis boots with `--requirepass` and refuses to start without it.

**Backend** is the heart of Catalyst: Fastify API server, WebSocket gateway for agent communication, plugin loader, task scheduler, alert service, and webhook dispatcher. (SFTP is served by the node agent on port `2022`, not by this stack.) It auto-runs database migrations on startup.
...
|---------|-------|----------|---------------|
| `postgres` | `pg_isready -U catalyst -d catalyst_db` | 10s | Backend returns 503, frontend proxy paths fail |
| `redis` | `REDISCLI_AUTH=... redis-cli ... ping \| grep PONG` | 10s | Redis container restarts; backend degrades to Postgres + local state |
| `backend` | `curl -sf http://localhost:3000/health` | 15s (`start_period: 60s`) | Frontend `depends_on backend healthy` delays frontend start |

The backend health check also verifies database connectivity. If PostgreSQL is unreachable, the backend returns HTTP 503. Frontend nginx still serves the static shell, but `/api` and `/ws` proxy paths fail.
...
2. **Pin in `.env` (requires a Compose edit — stock `docker-compose.yml` has no `${BACKEND_IMAGE}` variable):**
