...
**Redis** runs in the Compose stack but is currently inert: the backend has no Redis client (`REDIS_URL` is never read). The Docker stack still requires `REDIS_PASSWORD` (generated by `install.sh`); Redis boots with `--requirepass` and refuses to start without it. Do not debug auth or rate-limit issues via Redis.
...
| `redis` | `REDISCLI_AUTH=... redis-cli ... ping \| grep PONG` | 10s | Redis container restarts (backend itself does not use Redis) |
