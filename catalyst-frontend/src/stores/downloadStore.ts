...
/**
 * Tracks in-flight file downloads so progress stays visible even when the
 * user navigates elsewhere in the app.
 *
 * Mirrors the upload store: a "session" groups one user action (usually a
 * single file download). Sessions survive navigation and support canceling
 * in-flight downloads via the store's abort registry.
 */
export interface DownloadFileState {
  /** Path of the file on the server, used for display. */
  path: string;
  name: string;
  /** Bytes received so far. */
  loaded: number;
  /** Total bytes, when known (file listing size or Content-Length). */
  total?: number;
...
interface DownloadState {
  ...
  beginSession: (
    files: Array<Pick<DownloadFileState, 'path' | 'name' | 'total'>>,
  ) => string;
...
export const COMPLETED_DOWNLOAD_TTL_MS = 5000;

/**
 * Abort controllers for in-flight downloads, keyed by session id. Kept in a
...
const nextSessionId = () => {
  sessionCounter += 1;
  return `download-${Date.now()}-${sessionCounter}`;
};

/** Derives coarse session status from per-file statuses. */
const recomputeSessionStatus = (session: DownloadSession): DownloadSession['status'] => {
...
const mapSession = (
  sessions: DownloadSession[],
  sessionId: string,
  fn: (session: DownloadSession) => DownloadSession,
): DownloadSession[] =>
...
export const resetDownloadStoreForTests = () => {
  useDownloadStore.setState({ sessions: [] });
  abortRegistry.clear();
  sessionCounter = 0;
};
