// src/plugins/ticketing-plugin/components/admin/ServerTab.tsx
// Server-scoped ticket view. Same as AdminDashboard but filtered by serverId.
...
export function ServerTab({ serverId, serverName }: ServerTabProps) {
  ...
  // Override filters to always include serverId
  useEffect(() => {
    if (data.filters.serverId !== serverId) {
      data.setFilters({ ...data.filters, serverId });
    }
  ...
  // ── Create ticket (pre-fill server) ──
  async function handleCreateTicket(payload: Parameters<typeof actions.createTicket>[0]) {
    try {
      const ticket = await actions.createTicket({ ...payload, serverId });
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="flex items-center gap-3">
            <h1 className={cn('text-xl font-bold text-foreground', FONT_DISPLAY)}>
              {serverName ?? 'Server'} Tickets
            </h1>
            <span className={cn('text-sm', TEXT_MUTED)}>
              {data.ticketTotal} ticket{data.ticketTotal !== 1 ? 's' : ''}
            </span>
          </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              {data.isLoadingTickets ? (
                ...
                <EmptyState
                  icon={Ticket}
                  title="No tickets for this server"
                  description="There are no tickets linked to this server yet."
