...
function DangerZone() {
 ...
 {step === 0 ? (
 <Button variant="destructive" size="sm" onClick={() => setStep(1)} className="h-8 gap-1.5 px-3 text-mini">
 <Trash2 className="h-3.5 w-3.5" /> {t('dangerZone.delete')}
 </Button>
 ) : (
 <div className="flex items-center gap-2">
 <Input value={confirmText} onChange={(e) => setConfirmText(e.target.value)} placeholder={t('dangerZone.confirmPlaceholder')} className="h-8 w-36 rounded-sm border-border/60 bg-background/40 px-2.5 text-mini" />
 <Button variant="destructive" size="sm" disabled={confirmText !== 'DELETE' || deleteMutation.isPending} onClick={() => deleteMutation.mutate()} className="h-8 px-3 text-mini">
 {deleteMutation.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : t('common:actions.confirm')}
 </Button>
 <Button variant="ghost" size="sm" onClick={() => { setStep(0); setConfirmText(''); }} className="h-8 px-3 text-mini">{t('common:actions.cancel')}</Button>
 </div>
 )}
 </div>
 </ServerTabCard>
...
export default function ProfilePage() {
 ...
 <Button size="sm" onClick={() => setPwMutation.mutate()} disabled={!setPwVal || setPwMutation.isPending} className="h-8 w-full px-3 text-mini">
 ...
 <Button variant="outline" size="sm" onClick={() => disableTfaMutation.mutate()} disabled={!tfaPw || disableTfaMutation.isPending} className="text-danger hover:text-danger hover:bg-danger/5">{t('common:actions.disable')}</Button>
 </>)}
 </div>
 </div>
 </ServerTabCard>
 ...
 <div className="space-y-3">
 <div className="flex gap-2">
 <Input value={pkName} onChange={(e) => setPkName(e.target.value)} placeholder={t('passkeys.namePlaceholder')} className="h-8 flex-1 rounded-sm border-border/60 bg-background/40 px-2.5 text-mini" />
 ...
 passkeys.map((pk) => (
 <div key={pk.id} className="flex items-center justify-between border-b border-border/50 py-2 last:border-b-0 hover:bg-surface-1/40">
 <div className="flex items-center gap-2.5 min-w-0">
 <Smartphone className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
 {editPkId === pk.id ? (
 <Input value={editPkName} onChange={(e) => setEditPkName(e.target.value)} className="h-7 w-40 rounded-sm border-border/60 bg-background/40 px-2 text-mini" autoFocus />
 ) : (
 <div className="min-w-0">
 <div className="text-mini font-medium text-foreground truncate">{pk.name || t('unnamed')}</div>
 <div className="text-micro text-muted-foreground">{pk.deviceType || t('passkeys.deviceFallback')} · {fmtDate(pk.createdAt)}</div>
 </div>
 )}
 </div>
 <div className="flex items-center gap-1 shrink-0">
 {editPkId === pk.id ? (
 <button onClick={() => updPkMutation.mutate()} disabled={!editPkName} className="flex h-7 items-center rounded-sm px-2 text-mini font-medium text-foreground hover:bg-surface-2">{t('common:actions.save')}</button>
 ) : (
 ...
 <Button key={p} variant="outline" size="sm" onClick={() => profileApi.linkSso(p).then(() => { queryClient.invalidateQueries({ queryKey: qk.profileSsoAccounts() }); queryClient.invalidateQueries({ queryKey: qk.profile() }); })} className="h-8 px-3 text-mini"><ExternalLink className="mr-1.5 h-3 w-3" />{t('linkedAccounts.link', { provider: p.toUpperCase() })}</Button>
 ))}
 </div>
 ...
 <div key={a.id} className="flex items-center justify-between border-b border-border/50 py-2 last:border-b-0">
 <span className="text-mini font-medium text-foreground">{a.providerId.toUpperCase()}</span>
  <button onClick={() => profileApi.unlinkSso(a.providerId, a.id).then(() => queryClient.invalidateQueries({ queryKey: qk.profileSsoAccounts() }))} className="text-mini text-danger hover:text-danger/80">{t('linkedAccounts.unlink')}</button>
 </div>
 ))
 )}
 </div>
 ...
 {(sessions?.length ?? 0) > 1 && (
 <Button variant="outline" size="sm" onClick={() => revokeAllMutation.mutate()} disabled={revokeAllMutation.isPending} className="mt-2 h-8 w-full gap-1.5 px-3 text-mini">
 ...
 {apiKeys.length > 5 && (
 <Link to="/admin/api-keys" className="block py-1 text-center text-mini text-muted-foreground hover:text-foreground">{t('apiKeysCard.viewAll', { count: apiKeys.length })}</Link>
 )}
 </div>
 ) : (
 <TabEmptyState title={t('apiKeysCard.emptyTitle')} description={t('apiKeysCard.emptyDescription')} />
 )}
 <Link to="/admin/api-keys"><Button variant="outline" size="sm" className="mt-2 h-8 w-full gap-1.5 px-3 text-mini"><KeyRound className="h-3.5 w-3.5" />{t('apiKeysCard.manage')}</Button></Link>
 </ServerTabCard>
 ...
 <DialogBody>
 {qrValue && <div className="mb-4 flex justify-center"><img src={qrValue} alt={t('twoFactor.qrAlt')} className="rounded-sm border border-border/50 bg-surface-0 p-3" /></div>}
 {tfaSetup?.otpAuthUrl && <a href={tfaSetup.otpAuthUrl} target="_blank" rel="noopener noreferrer" className="mb-3 flex items-center gap-1 text-mini font-medium text-primary hover:text-primary/80"><ExternalLink className="h-3 w-3" /> {t('twoFactor.openInApp')}</a>}
 {tfaSetup?.secret && (
 <div className="mb-3 rounded-sm border border-border/50 px-3 py-2 text-center">
 <div className="type-overline">{t('twoFactor.manualKey')}</div>
 <code className="mt-1 block font-mono text-data font-semibold tabular-nums text-foreground select-all">{tfaSetup.secret}</code>
 <button onClick={() => { navigator.clipboard.writeText(tfaSetup?.secret ?? ''); notifySuccess(t('common:actions.copied')); }} className="mx-auto mt-2 flex h-7 items-center gap-1 rounded-sm px-2 text-micro text-muted-foreground hover:text-foreground"><Copy className="h-3 w-3" /> {t('common:actions.copy')}</button>
 </div>
 )}
 {(tfaSetup?.backupCodes?.length ?? 0) > 0 && (
 <div className="mb-3 rounded-sm border border-warning/30 bg-warning/5 px-3 py-2">
 <div className="mb-2 flex items-center gap-1.5 text-mini font-semibold text-warning"><AlertTriangle className="h-3.5 w-3.5" /> {t('twoFactor.backupCodesWarning')}</div>
 <div className="grid grid-cols-2 gap-1.5">{tfaSetup!.backupCodes!.map((code) => <code key={code} className="rounded-sm border border-border/50 px-2 py-1 text-center font-mono text-micro tabular-nums">{code}</code>)}</div>
 </div>
 )}
 </DialogBody>
 <DialogFooter>
