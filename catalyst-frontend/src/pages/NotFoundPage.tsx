...
function NotFoundPage() {
  ...
  return (
    <div className="app-shell relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 text-center text-foreground dark:text-zinc-100">
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-zinc-50 via-white to-zinc-100 dark:from-zinc-950 dark:via-zinc-950 dark:to-zinc-900" />
      <div className="relative z-10 grid w-full max-w-5xl gap-8 md:grid-cols-[1.1fr_1fr]">
        <div className="flex flex-col items-center gap-4 text-center md:items-start md:text-left">
          <p className="text-sm uppercase tracking-widest text-muted-foreground dark:text-muted-foreground dark:text-muted-foreground">
            404 • Sector offline
          </p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-xs">
            <div className="rounded-lg border border-border bg-white px-3 py-2 text-xs text-muted-foreground dark:border-border dark:bg-surface-1 dark:text-zinc-300">
              Score: <span className="font-semibold text-foreground dark:text-zinc-100">{score}</span>
            </div>
            <div className="rounded-lg border border-border bg-white px-3 py-2 text-xs text-muted-foreground dark:border-border dark:bg-surface-1 dark:text-zinc-300">
              Power: <span className="font-semibold text-foreground dark:text-zinc-100">{power}</span>
            </div>
            <div
                ...
                displayedMessage === 'hit'
                  ...
                  : displayedMessage === 'miss'
                    ...
                    : displayedMessage === 'timeout'
                      ...
                      : 'border-border bg-white text-muted-foreground dark:border-border dark:bg-surface-1 dark:text-zinc-300'
              }`}
            >
              {displayedMessage === 'hit'
                ? 'Direct hit!'
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-xs">
            <button
              type="button"
              className="rounded-lg border border-border bg-white px-4 py-2 text-sm font-semibold text-muted-foreground transition-all duration-300 hover:border-primary-500 hover:text-foreground dark:border-border dark:bg-surface-1 dark:text-zinc-300 dark:hover:border-primary/30"
              onClick={() => {
                setScore(0);
        ...
        <div className="flex items-center justify-center">
          <div className="grid grid-cols-8 gap-2 rounded-2xl border border-border bg-white/70 p-5 shadow-surface-light backdrop-blur dark:border-border dark:bg-zinc-950/70 dark:shadow-surface-dark">
            {cells.map((index) => {
                ...
                <button
                  ...
                  className={`h-6 w-6 rounded-md border transition-all duration-300 ${
                    isTarget
                      ...
                      : 'border-border bg-white hover:border-primary-300 dark:border-border dark:bg-surface-1 dark:hover:border-primary-500/40'
                  }`}
