...
const ROUTES: RouteDef[] = [
  ...
  // Plugin Manager
  { file: 'servers.ts', method: 'GET', path: '/:serverId/plugin-manager/search', summary: 'Search plugins', description: 'Search for plugins. Sort: trending, popular, rating, updated, newest (default: trending without query, relevance with query).', paramsSchema: { type: 'object', properties: { serverId: { type: 'string' } }, required: ['serverId'] }, querySchema: { type: 'object', properties: { query: { type: 'string' }, gameVersion: { type: 'string' }, sort: { type: 'string', enum: ['trending', 'popular', 'rating', 'updated', 'newest'] }, page: { type: 'integer' } } }, responseCodes: [200], permissions: 'server.read' },
  ...
  { file: 'templates.ts', method: 'POST', path: '/import-pterodactyl-batch', summary: 'Import all Pterodactyl eggs', description: 'Download and import all eggs from pterodactyl/game-eggs GitHub repo', bodySchema: { type: 'object', properties: { nestId: { type: 'string' }, repoUrl: { type: 'string' } } }, responseCodes: [200, 502], permissions: 'template.create' },
