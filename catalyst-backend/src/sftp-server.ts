...
const SERVER_FILES_ROOT =
	process.env.SERVER_DATA_DIR || "/var/lib/catalyst/servers";

/** Maximum file size for SFTP write operations (default 100MB) */
const SFTP_MAX_FILE_SIZE = parseInt(
	process.env.SFTP_MAX_FILE_SIZE || String(100 * 1024 * 1024),
);

// Generate or load host key
const HOST_KEY_PATH =
	process.env.SFTP_HOST_KEY || "/tmp/catalyst-sftp-host-key";
let hostKey: Buffer;
...
async function validateTokenAndGetServer(
	username: string,
	password: string,
): Promise<SFTPSession | null> {
	try {
			...
			const resetIdleTimer = () => {
				...
				idleTimer = setTimeout(() => {
					logger.info(
						...
						"SFTP session idle timeout, disconnecting",
					);
					client.end();
				}, IDLE_TIMEOUT_MS);
			};
...
function handleSFTPSession(sftpStream: SFTPStream, session: SFTPSession) {
	...
	sftpStream
			...
			enqueue(async () => {
				try {
					...
					const fileList: Array<{
						...
						{
							filename: ".",
							longname: formatLongname(".", dirStats),
							attrs: dirAttrs,
						},
						{
							filename: "..",
						...
						try {
							...
							if (stats.isSymbolicLink()) {
								try {
									...
									const targetOk =
										realTargetPath === session.realServerPath ||
		...
		.on("RENAME", (reqid, oldPath, newPath) => {
			enqueue(async () => {
				try {
					const fullOldPath = normalizePath(
						session.serverPath,
						session.realServerPath,
						oldPath,
					);
					const fullNewPath = normalizePath(
						session.serverPath,
						session.realServerPath,
						newPath,
					);
