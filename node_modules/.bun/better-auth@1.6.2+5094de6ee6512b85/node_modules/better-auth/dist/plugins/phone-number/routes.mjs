...
const sendPhoneNumberOTP = (opts) => createAuthEndpoint("/phone-number/send-otp", {
	...
	if (ctx.context.options.advanced?.backgroundTasks?.handler && sendOTPResult instanceof Promise) try {
		ctx.context.runInBackground(sendOTPResult.catch((e) => {
			ctx.context.logger.error("Failed to run background task:", e);
		}));
	} catch (e) {
		ctx.context.logger.error("Failed to run background task:", e);
	}
	else await sendOTPResult;
...
const verifyPhoneNumber = (opts) => createAuthEndpoint("/phone-number/verify", {
	...
	if (!user) {
		if (opts?.signUpOnVerification) {
			const { phoneNumber, code, disableSession, updatePhoneNumber, ...rest } = ctx.body;
...
const resetPasswordPhoneNumber = (opts) => createAuthEndpoint("/phone-number/reset-password", {
	...
	if (!userRes) throw APIError.from("BAD_REQUEST", PHONE_NUMBER_ERROR_CODES.UNEXPECTED_ERROR);
	...
	if (!accounts.find((account) => account.providerId === "credential")) await ctx.context.internalAdapter.createAccount({
		userId: user.id,
